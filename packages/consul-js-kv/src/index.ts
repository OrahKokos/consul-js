...
export const resolveWatchOptions = (
  options?: ConsulWatchOptions,
): ConsulWatchOptions =>
  options
    ? options
    : {
...
export const getKV =
  ...
  async (consulKeyPath: string, query: GetKVQueryObject = {}) => {
    const options = resolvers.requestOptionsResolver('GET')(
      resolvers.pathResolver(consulKeyPath),
    )({
      ...query,
      recurse: false,
    })
...
export const delKV =
  ...
  async (consulKeyPath: string, query: DeleteKVQueryObject = {}) => {
    const options = resolvers.requestOptionsResolver('DELETE')(
      resolvers.pathResolver(consulKeyPath),
    )({
      ...query,
      recurse: false,
...
export const init =
  (options: ConsulOptions) => (requestHandler: RequestHandler) => {
...
export const initWithResolver =
  (WatchInitOptions: WatchInitOptions) =>
  (topLevelResolver: ConsulTopResolver) => {
    const resolvers = {
      ...topLevelResolver,
