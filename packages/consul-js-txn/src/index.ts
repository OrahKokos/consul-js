import { ConsulTopResolver, ConsulModuleResolver, ConsulOptions, RequestHandler, defaultInit } from 'consul-js-common'
...
const createTxnKVGetPayload = (consulKey: string): TxnKVNamespace<TxnKVGet> => ({
  KV: {
    Verb: 'get',
    Key: consulKey
  }
...
const createTransaction =
  ...
  async (data: TxnData, query: TxnQueryObject = {}) => {
    const options = resolvers.requestOptionsResolver('PUT')(resolvers.pathResolver())(query)
...
export { init, initWithResolver, createTransaction, createTxnKVGetPayload, TxnData }
