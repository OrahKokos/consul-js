...
describe('Consul common package', () => {
  describe('getPath', () => {
    it('Should return version and service only if last arg is empty', () => {
      ...
      expect(getPath(CONSUL_VERSION_PATH)(SOME_SERVICE)()).toEqual(expectedResult)
      expect(getPath(removeSlash(CONSUL_VERSION_PATH))(removeSlash(SOME_SERVICE))()).toEqual(expectedResult)
    })
    it('Should return full path only if last arg is not empty', () => {
      const expectedResult = `${CONSUL_VERSION_PATH}${SOME_SERVICE}${SOME_PATH}`
      expect(getPath(CONSUL_VERSION_PATH)(SOME_SERVICE)(SOME_PATH)).toEqual(expectedResult)
      expect(getPath(removeSlash(CONSUL_VERSION_PATH))(removeSlash(SOME_SERVICE))(removeSlash(SOME_PATH))).toEqual(
        expectedResult
      )
    })
  })
  ...
  describe('resolveWithQueryParams', () => {
    it('Should resolve path with query params if query params object is not empty', () => {
      ...
      expect(resolveWithQueryParams(SOME_QUERY)(SOME_PATH)).toEqual(expectedResult)
    })
  ...
  describe('resolveRequestOptions', () => {
    it('It should always resolve with no issue', () => {
      const x = resolveRequestOptions(SOME_OPTIONS)('GET')(SOME_PATH)(SOME_QUERY)
  ...
  describe('wrapErrorHandler', () => {
    it('Should not emit error on no throw', async () => {
      ...
      events.on('error', spy)
      const options = resolveRequestOptions(SOME_OPTIONS)('GET')(SOME_PATH)(SOME_QUERY)
